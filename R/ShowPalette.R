...
#' @export
#' @examples
#' showPalette(c("#005000","#008600","#00BB00"), border="light gray")

remcolor.showPalette <- function(col, border = NA, ...){
    n <- length(col)
...
remcolor.showPalettes <- function(col_list, border = NA, show.rgb=T, ...){
    nr <- length(col_list)
    x_off <- -0.01
    plot(0, 0, type="n", xlim = c(x_off, 1), ylim = c(0, 1),
         axes = FALSE, xlab = "", ylab = "", ...)
    ...
    if (! is.null(names(col_list))) {
        text(x=x_off, y=yi, adj=c(1,1.1), col='black', labels=names(col_list))
    } else {
        text(x=x_off, y=yi, adj=c(1,1.1), col='black', labels=1:length(col_list))
    }
}
...
remcolor.showPaletteColorblind <- function(col, border = NA, ...){
    cb.red <- remcolor.simulate_colorblind_red(col)
    cb.green <- remcolor.simulate_colorblind_green(col)
    cb.blue <- remcolor.simulate_colorblind_blue(col)
    remcolor.showPalettes(
        list(O=col, R=cb.red, G=cb.green, B=cb.blue),
        border=border
    )
}
...
#' remcolor.plotPalettes(a, border="light gray")

remcolor.plotPalettes <- function(col_list, border = NA, ...){
...
#     col.df$intensity <- as.numeric(col.df$intensity)
    ...
    g <- ggplot(col.df, aes(x=x, y=intensity,
                            color=channel,
                            group=interaction(channel,color)))
    g <- g + geom_line(aes(size=4)) +
        scale_y_continuous(breaks=c(seq(0, 255, 32), 255), limits=c(0,255)) +
